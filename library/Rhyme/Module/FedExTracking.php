...
use Isotope\Isotope;
use Isotope\Module\Module as ModuleIsotope;

class FedExTracking extends ModuleIsotope
{
	 ...
	 * @var string
	 */
	protected $strTemplate = 'mod_iso_fedextracking';

	protected $strFormId = 'iso_mod_fedex_tracking';
	...
	public function generate()
	{
		if (TL_MODE == 'BE')
		{
			...
			$objTemplate->wildcard = '### ISOTOPE ECOMMERCE: FEDEX TRACKING ###';
	...
	/** 
	 * Simplify the nightmarish bloatteriffic FEDEX response
	 */
	protected function parseResponse($arrResponse)
	{
		...
		switch($arrResponse['Response']['ResponseStatusCode'])
		{
			...
			default:
				...
				$arrAddressData = $this->parseStupidFEDEXAddress($arrResponse['Shipment']['ShipTo']['Address']);
				...
				{
					$arrAddressData = $this->parseStupidFEDEXAddress($row['ActivityLocation']['Address']);
				
					$arrActivities[] = array
					(
						'datim'			=> $this->parseStupidFEDEXDatim($row['Date'],$row['Time']),
	...
	protected function parseStupidFEDEXAddress($arrData)
	{
	 ...
	 */
	protected function parseStupidFEDEXDatim($strDate,$strTime='000000')
	{
		//YYYYMMDD // 00:00:00
	...
	{				
		...
		if(!$strIds)
		{
			$this->Template->message = $GLOBALS['TL_LANG']['MSC']['noEnabledFedExServices'];
			return array();
		}
		
		$objFEDEXService = $this->Database->prepare("SELECT fedex_enabledservice FROM tl_iso_shipping_modules WHERE id IN($strIds) AND type='fedex' OR type='multi_fedex'")->limit(1)->execute();
		
		if(!$objFEDEXService->numRows)
		{
			$this->Template->message = $GLOBALS['TL_LANG']['MSC']['noEnabledFedExServices'];
			return array();
		}
		...
		//Service Info//
		$arrShipment['pickup_type'] = (string)$objFEDEXService->fedex_enabledservice;	//default one-time but should be configurable.
		$arrShipment['service'] = (string)$objFEDEXService->fedex_enabledservice; //ground
		...
		//END Service Info/
		$objFedExTimeRequest = new FedExAPITimeInTransit($arrOrigin, $arrFieldMap, $arrData);

		$xmlTime = $objFedExTimeRequest->buildRequest();

		$arrReturn['transit'] = $objFedExTimeRequest->sendRequest($xmlTime);
		...
		$objFedExRateRequest = new FedExAPIRatesAndService($arrShipment, $arrOrigin, $arrOrigin, $arrFieldMap);		
			
		$xmlRate = $objFedExRateRequest->buildRequest();
				
		
		// check the output type
		
	
		$arrReturn['rates']	= $objFedExRateRequest->sendRequest($xmlRate);
